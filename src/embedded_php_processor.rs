...
impl EmbeddedPhpProcessor {
    ...
    pub fn new() -> Result<Self> {
        ...
        builtin_functions.insert(
            ...
            Self::php_version as fn(&[String]) -> String,
        );
        builtin_functions.insert(
            "date".to_string(),
            Self::php_date as fn(&[String]) -> String,
        );
        builtin_functions.insert(
            "time".to_string(),
            Self::php_time as fn(&[String]) -> String,
        );
        builtin_functions.insert(
            "strlen".to_string(),
            Self::php_strlen as fn(&[String]) -> String,
        );
        builtin_functions.insert(
            "strtoupper".to_string(),
            Self::php_strtoupper as fn(&[String]) -> String,
        );
        builtin_functions.insert(
            "strtolower".to_string(),
            Self::php_strtolower as fn(&[String]) -> String,
        );
        builtin_functions.insert(
            "htmlspecialchars".to_string(),
            Self::php_htmlspecialchars as fn(&[String]) -> String,
        );

        debug!(
            "Simple PHP processor initialized with {} built-in functions",
    ...
    fn process_php_code(&self, code: &str, context: &mut PhpContext) -> Result<String> {
        ...
        let php_tag_regex =
            PHP_TAG_RE.get_or_init(|| Regex::new(r"(?s)<\?(?:php)?\s*(.*?)\?>").unwrap());
                ...
                && s.ends_with(')')
            {
                ...
                let args: Vec<String> = if args_str.trim().is_empty() {
                    ...
                    args_str
                        .split(',')
                        .map(|arg| {
                            self.evaluate_expression(arg.trim(), context)
                                .unwrap_or_default()
                        })
                        .collect()
                };
    ...
    fn handle_if_statement(&self, code: &str, context: &mut PhpContext) -> Result<Option<String>> {
        let if_regex =
            IF_RE.get_or_init(|| Regex::new(r"(?s)if\s*\(\s*(.+?)\s*\)\s*:(.*?)endif").unwrap());
    ...
    fn handle_foreach_loop(&self, code: &str, context: &mut PhpContext) -> Result<Option<String>> {
        let foreach_regex = FOREACH_RE.get_or_init(|| {
            Regex::new(r"(?s)foreach\s*\(\s*(.+?)\s+as\s+(.+?)\s*\)\s*:(.*?)endforeach").unwrap()
        });
...
mod tests {
    ...
    fn test_simple_php_execution() {
        ...
        let result = processor
            .execute_php(php_code, &get_params, &post_params, &server_vars)
            .unwrap();
